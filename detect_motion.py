...
if args.get("video") is None:
    resolution = (640, 480)
    #   camera = PiVideoStream(resolution)
    vs = camera
    motionName = datetime.datetime.now().strftime("%d-%M-%Y_%H_%M_%S.mp4")
else:
    vs = cv2.VideoCapture(filename)
    motionName = "detect_" + os.path.basename(filename)

if args.get("output") is not None:
    motionName = args.get("output") + "/" + motionName
...
index = 0
print(f"Writing {motionName}")
out = None
scale = 1
display = args.get("display")
if not args.get("noop"):
    out = cv2.VideoWriter(motionName, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), 30, resolution)
...
while True:
    ...
    counter = counter + 1
    #   if abs(timestamp - datetime.datetime.now().second) > 10 :
    #       timestamp = datetime.datetime.now().second
    #       fps = counter / 10
    #       counter = 0
    ...
    #        cv2.imwrite(motionName + ".jpg")
    #        break

    #    currentTs = datetime.datetime.now()
    #    delta = currentTs - lastVideoTs
    #    if delta.total_seconds() > 3600:
    #        if out is not None:
    #            out.release()
    #        motionName = datetime.datetime.now().strftime("%d-%M-%Y_%H_%M_%S.mp4")
    #        out = cv2.VideoWriter(motionName, cv2.VideoWriter_fourcc('m','p','4','v'), 30, resolution)
    #        lastVideoTs = currentTs
    #        print(f"Create new output {motionName}")
    ...
    if counter % QUANTISATION == 0:
        ...
        cv2.putText(disp, f"Filtered: {int(fs)}", org=(20, 100), fontFace=cv2.FONT_HERSHEY_SIMPLEX, color=(0, 0, 255),
                    thickness=3, fontScale=1.5)
        ...
        if st > area_limit and fs > filtered_limit:
            ...
            for i in numpy.arange(0, detections.shape[2]):
                ...
                if confidence > args["confidence"]:
                    ...
                    detected = True
        if detected and display:
            cv2.imshow("AI", copy)

        #        if detected and motion == 0:
        #            cv2.putText(disp, f"Prepare!", org = (20, 500),fontFace=cv2.FONT_HERSHEY_SIMPLEX, color=(0,0, 255),thickness=3, fontScale=2)
        #            cv2.imshow("TEST", disp)
        #            cv2.waitKey(0)
        if detected:
